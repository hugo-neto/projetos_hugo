...
def coleta_excel(cond=False):
    while cond==False:
        ...
        try:
            ...
            df = pd.read_excel(os.path.join(os.path.abspath('.'),"Dados2.xls"))
            cond = True
        except:
...
def arranjaLista(file):
    ...
    for i, linha in enumerate(conteudo):
        ...
        if i == 4:
            VetorLinha = linha.replace("[", "").replace("]", "").replace("\t", "").replace("\n", "").split("/")
            vetorLinhaBaixo = conteudo[5].replace("\t", "").replace("\n", "").replace("[", "").replace("]", "").split("/")
            #>>> VetorLinha, vetorLinhaBaixo
            #(['ABCDE', 'FGH'], ['F', 'GH'])
            # Se o corte foi feito correto:

            if VetorLinha[0][0] == vetorLinhaBaixo[0][0]:
                pedaco = conteudo[i+1:i+len(VetorLinha[0])]
                final = conteudo[i+len(VetorLinha[0]):]

                for j, novaLinha in enumerate(pedaco):
                    novaLinha = novaLinha.replace("\t", "").replace("\n", "").replace("[", "").replace("]", "")
                    novaLinha = "\t"*(len(VetorLinha[0])+3-len(novaLinha)) + "[" + novaLinha + "]\n"
                    novoTexto = novoTexto + novaLinha
                
                for k, novaLinha in enumerate(final):
                    
                    novaLinha = novaLinha.replace("\t", "").replace("\n", "").replace("[", "").replace("]", "")
                    novaLinha = "\t"*(len(VetorLinha[1])+3-len(novaLinha)) + "[" + novaLinha + "]\n"
                    novoTexto = novoTexto + novaLinha
                arquivo = open(file, 'w')
                arquivo.writelines(novoTexto)
                arquivo.close()
                return
            
            # Se não foi:
            elif VetorLinha[0][0] != vetorLinhaBaixo[0][0]:
                final = conteudo[i+1:i+len(VetorLinha[1])]
                pedaco = conteudo[i+len(VetorLinha[1]):]
                print(final)
                print(pedaco)
...
def main():
    ...
    try:
        ...
        print("Problema ao ler o arquivo. Verifique:")
        print("1 - Nome do excel é 'Dados.xls'")
        print("2 - Se o excel está no mesmo diretório de execução do script")
        print("3 - Se todos os pacotes estão instalados: pandas, numpy, xlrd")
        sys.exit()
    # 2. Trata dados
    # Cria txt
...
"""
    ...
    arranjaLista("resultado_HugoNeto.txt")

    # Acessa o txt e lê as linhas para poder printar na command line
    resultado = open("resultado_HugoNeto.txt", "r")
